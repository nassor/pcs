...
// rows each. Everything else — the systems, the DAG, the stage plan — is
// identical. This benchmark holds the total row count fixed and sweeps k, so
// the only variable is item size.
//
// Two groups:
//
...
//      swept across `STAGE_INLINE_THRESHOLD` (1024 rows). Below the threshold
//      the executor runs them inline; at or above it dispatches one
//      `spawn_blocking` per system. The discontinuity at the boundary is what
//      dispatch costs, and shows whether the gate is placed sensibly.
//
// The accumulator lives in an `Arc<Mutex<f64>>` inside the system struct, not
// in a `Dataset` resource, because stream mode clears the dataset between items
// and `clear()` drops resources. That is the documented cross-item state
// contract for `run_stream`, and asserting batch total == stream total is how
// this benchmark checks the two modes agree.
...
/// almost nothing to the measurement — unlike a channel, which would add
/// wakeup latency to every item.
struct ReplaySource {
...
fn bench_batch_vs_stream(c: &mut Criterion) {
    ...
    // 20 samples: rebuilding a 100k-row dataset per iteration is allocator-
    // sensitive, and 10 samples left the batch point with a 2x confidence
    // interval.
    ...
    // Swept well past `STAGE_INLINE_THRESHOLD` to locate the row count at which
    // one `spawn_blocking` per system actually starts paying for itself.
    // Neither system implements `run_slice`, so slice parallelism never
    // applies here and this isolates stage-level dispatch alone.
