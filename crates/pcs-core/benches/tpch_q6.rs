...
fn bench_q6(c: &mut Criterion) {
    ...
    // Isolates the per-iteration dataset construction inside `narrow_pcs` and
    // `wide_pcs`: `build_*_pipeline` allocates and zero-fills a fresh 1M-row
    ...
    // `narrow_pcs` is framework overhead.
