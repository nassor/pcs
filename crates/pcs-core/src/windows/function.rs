...
//!
//! - **`WindowFunction::Reduce`** – applies a built-in columnar aggregate
//!   (`Sum`, `Min`, `Max`, `Count`, `Mean`) over a single numeric field
//!   within each window group.
//!
//! - **`WindowFunction::Process`** – delegates to a user-supplied
...
pub trait ProcessWindowFn: Send + Sync {
    ...
    /// # Arguments
    ///
    /// * `ctx` – contextual metadata: window boundaries, late-firing flag, and
    ///   current watermark.
    /// * `batch` – all rows assigned to this window group, in their original
    ///   order.
    ///
    /// # Returns
    ///
