//! Fixtures shared by the `scheduler_etl*` integration tests.
//!
//! `scheduler_etl.rs` and `scheduler_etl_parallel.rs` exercise the same ETL
//! workload through the sequential and the `ParallelSystem` stage paths, so the
//! component, the FX table, the report resource, and the seed rows live here
//! once. Each test binary compiles its own copy of this module, so items an
//! individual test does not touch would otherwise warn.
#![allow(dead_code)]
...
///
/// Four are rejected by `ValidateSystem` (`id` 1004 negative, 1008 zero) or
/// left unconverted; the assertions in each test depend on these exact values.
