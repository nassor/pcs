...
//! `pcs_processor::export_pipeline!` against `wit-bindgen`-generated bindings.
//!
...
//! --release -p pcs-processor-smoketest --target wasm32-wasip2`.
...
use pcs_processor::{ProcessorState, RouteDecision};
use std::sync::Arc;
...
fn count_batches(data: &mut Dataset) -> Result<(), PcsError> {
    let state = data
        ...
        .ok_or_else(|| PcsError::generic("smoketest: ProcessorState<Counter> resource missing"))?;
...
pub fn build() -> Pipeline {
    ...
    #[cfg(target_arch = "wasm32")]
    let route = pcs_config_get(ROUTE_KEY);
    #[cfg(not(target_arch = "wasm32"))]
