//! Drive one PCS workload through four WebAssembly guests written in four
//! different languages.
//!
//! Every stage is a separate `.wasm` component implementing the same
//! `pcs:pipeline@0.2.0` WIT world, loaded through the same host
//! (`WasmPipelineRuntime`). The languages differ; the contract does not.
//!
//! | order | component        | language   | writes                  |
//! |-------|------------------|------------|-------------------------|
//! | 1     | `validate-go`    | Go         | `valid`                 |
//! | 2     | `enrich-py`      | Python     | `usd_amount`            |
//! | 3     | `score-js`       | JavaScript | `risk_score`, `flagged` |
//! | 4     | `settle-rs`      | Rust       | `settlement` + ledger   |
//!
//! # Modes
//!
//! ```bash
//! # 1. regenerate examples/polyglot/generated/ (schema bytes, fingerprint, fixtures)
//! cargo run -p pcs-service --features wasm --example polyglot_orders -- emit
//!
//! # 2. run the four-language chain (needs `bash scripts/build-polyglot.sh` first)
//! cargo run -p pcs-service --features wasm,tracing --example polyglot_orders
//! ```
//!
//! Use `--features wasm,tracing` for the run: without the `tracing` feature the
//! host's `host-io::log` bridge falls back to `eprintln!` and `host-io::metric`
//! is dropped on the floor, so the guests' metrics stay invisible.
//!
//! # Why an `emit` mode exists
//!
//! The Go, Python and JavaScript guests do not link arrow-rs, and they never
//! *write* a flatbuffer. They need two constants — the canonical `Order` Arrow
//! IPC schema bytes and the schema fingerprint — which `emit` derives from
//! `pcs_polyglot_order::Order` and writes out as one generated source file per
//! language. `emit` also writes the shared five-row fixture in both `.pcs`
//! (host wire format) and `.json` (expected values) form so each language's
//! native codec test can check itself without a WebAssembly toolchain.
//!
//! Drift is not merely documented against: `run` mode compares every stage's
//! reported `schema_fingerprint` against the live fingerprint of
//! `pcs_polyglot_order::Order` and exits non-zero on mismatch.
...
/// Epoch ticks granted to each guest call. 100 ticks × 100 ms = the same 10 s
/// budget `pcs-service`'s own WASM loader uses.
...
/// calls actually reach the terminal.
///
/// The host bridges `log` to `tracing` and `metric` to `tracing::trace!` when
/// `pcs-service` is built with the `tracing` feature — but a `tracing` event
/// with no subscriber goes nowhere, and metrics are at TRACE, which no default
/// filter passes. Hence the explicit default directive. `RUST_LOG` overrides it.
#[cfg(feature = "tracing")]
fn init_tracing() {
...
/// Built without the `tracing` feature: the host falls back to `eprintln!` for
/// `log` and drops `metric` on the floor. Nothing to install.
...
/// The 8-char lowercase hex fingerprint the WIT
/// `pipeline-descriptor.schema-fingerprint` field carries for a pipeline that
/// registers exactly `Order`.
...
/// schema and immediately finished, no batches. Byte-identical to what
...
// ---------------------------------------------------------------------------
// base64 — local so the generated files stay dependency-free
// ---------------------------------------------------------------------------

/// Standard base64 with `=` padding (RFC 4648 §4).
///
/// Hand-rolled rather than pulled in as a dependency: it is used once per
/// `emit` run to embed ~700 bytes of schema into three generated source files.
...
fn emit() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let js = format!(
        "// {GENERATED_BY}\n\n\
         /** Canonical `Order` Arrow IPC schema-message stream, base64-encoded. */\n\
         export const ORDER_SCHEMA_IPC_BASE64 = \"{b64}\";\n\n\
         /** Schema fingerprint reported in pipeline-descriptor.schema-fingerprint. */\n\
         export const ORDER_FINGERPRINT = \"{fingerprint}\";\n"
    );
...
/// The chain, in execution order. Order matters: each stage reads columns the
/// previous one wrote.
...
fn print_table(dataset: &Dataset) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  {:>2}  {:<6} {:<4} {:>12}  {:<5} {:>12} {:>10}  {:<7} {:<9}",
        "id", "region", "cur", "amount", "valid", "usd_amount", "risk", "flagged", "settlement"
    );
    for i in 0..orders.len() {
        println!(
            "  {:>2}  {:<6} {:<4} {:>12.2}  {:<5} {:>12.2} {:>10.4}  {:<7} {:<9}",
            id.value(i),
            region.value(i),
            currency.value(i),
            amount.value(i),
...
/// Read the ledger totals out of a `run-result.checkpoint` blob.
///
/// The blob is a standalone single-component Arrow IPC stream written by
/// `pcs-guest`'s `Stateful::capture`, so the host can decode it with the same
/// `Dataset::read_ipc` it uses for the data plane.
...
async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // ---- batch 2: same rows, prior threaded into the stateful stage ----
    ...
    for (stage, runtime) in &runtimes {
        // Only the stateful stage gets a `prior`; the other three must return
        // `None` regardless of what they are handed.
    ...
    println!(
        "\nOK — four languages, one WIT world. Only `settlement` came from Rust; \
         `valid` came from Go, `usd_amount` from Python, `risk_score`/`flagged` from JavaScript."
    );
    Ok(())
}
