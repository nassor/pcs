//! End-to-end Parquet ETL using the `arrow-io` Source/Sink layer.
//!
//! Generates a synthetic 10 000-row `input.parquet` in a temp directory, reads
//! it with [`ParquetSource`] through [`Pipeline::run_with_io`], scales `price`
//! by 1.1 and derives a `rounded` column with two [`ParallelSystem`]s, writes
//! the result with [`ParquetSink`], then reads it back and prints five rows.
