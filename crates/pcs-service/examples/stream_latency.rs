//! Per-item latency proof for stream mode.
//!
//! Two measurements, both end-to-end from the producer's point of view:
//!
//! 1. **Native.** A `Pipeline` with one system, driven by
//!    [`service::stream::run_stream`] over a `ChannelSource`/`ChannelSink` pair.
//!    The producer sends one single-row batch and waits for the transformed row
//!    to come back out of the sink, timing the whole loop.
//! 2. **WASM** (`--features wasm`). `WasmPipelineRuntime::run_on_with_state`
//!    against a one-row dataset, isolating the host↔guest boundary: store
//!    creation, instantiation from the pre-linked `InstancePre`, IPC in, IPC
//!    out.
//!
//! Run it:
//!
...
async fn native_latency() -> Vec<u64> {
    ...
    // Buffer 1 on both ends: the producer cannot run ahead of the pipeline, so
    // each sample is one full source→systems→sink traversal.
