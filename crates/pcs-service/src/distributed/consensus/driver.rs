...
pub(crate) mod raft_impl {
    ...
    impl ArrowRaftDriverHandle {
        ...
        /// # Errors
        ///
        /// Returns [`PcsError::Store`] when `listen_addr` cannot be bound.
        pub async fn spawn_tcp_server(
            &self,
            listen_addr: std::net::SocketAddr,
        ) -> PcsResult<tokio::task::JoinHandle<std::io::Result<()>>> {
            use crate::distributed::consensus::transport::RaftTcpServer;
            RaftTcpServer::new(self.raft.clone(), listen_addr)
                .bind_and_spawn()
                .await
                .map_err(|e| PcsError::store(format!("bind raft listener on {listen_addr}: {e}")))
        }
