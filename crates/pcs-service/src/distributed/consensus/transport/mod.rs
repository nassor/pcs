...
//! ## Layout
//!
//! This file owns the shared frame/snapshot constants and the
//! [`TransportError`] classification; the rest is split by direction.
//! `wire` holds the on-wire message types and the frame codec, `client` the
//! outbound half ([`TcpNetwork`], the per-peer pool, proposal forwarding), and
//! `server` the inbound half ([`RaftTcpServer`]).
//!
//! ## TCP server
//!
...
//! ## Connection pool
//!
//! [`TcpNetwork`] maintains a per-peer pool of idle
//! [`TcpStream`](tokio::net::TcpStream)s bounded by [`POOL_CAPACITY`]. Streams
//! are acquired from the pool (or freshly connected) and returned on success;
