...
//!
//! Both functions use [`GUEST_STATE_STAGE_SENTINEL`] as the `stage_idx` so the
...
//!
//! ## Keying, and why it is chained
//!
//! The blob is written under the **current** claim's id and read back under the
//! id of the claim that last wrote one. The runner keeps that pointer for the
//! duration of its loop.
//!
//! It cannot be a stable per-partition key: the redb/Raft state machine
//! resolves a checkpoint's `batch_id` from the CLAIMS table
//! (`apply_checkpoint`), so a write whose uuid is not a live claim is rejected
//! with "claim_id not found". Reads are a plain point lookup and survive the
//! claim being acked, so chaining works in the direction that matters.
//!
//! Consequence: the blob is durable, but the pointer to the newest one is not.
//! A runner that restarts hands the runtime `None` and it starts from a cold
//! state — the same contract the WIT `prior: option<checkpoint>` states for a
//! cold start. Nothing is silently mixed; state is simply rebuilt.
//!
//! ## Payload format
//!
//! There isn't one, as far as the host is concerned. The bytes are exactly what
//! [`PipelineRuntime::run_on_with_state`](pcs_core::runtime::PipelineRuntime::run_on_with_state)
//! returned and are handed back verbatim as the next call's `prior`. For a WASM
//! guest built with `pcs_guest::export_pipeline!(build, state = C)` they happen
//! to be a single-component Arrow IPC stream, but nothing here depends on that.
//!
//! ## First-run bootstrap
//!
//! [`load_guest_state`] returns `Ok(None)` when no prior checkpoint exists, or
//! when one exists with an empty payload. The caller passes `None` straight
//! through as `prior`, which the runtime reads as "cold start".

use uuid::Uuid;

use crate::PcsResult;
use crate::distributed::checkpoint::{CheckpointStore, GUEST_STATE_STAGE_SENTINEL};
...
pub async fn load_guest_state(
    store: &(impl CheckpointStore + ?Sized),
    prior_claim_id: Uuid,
) -> PcsResult<Option<Vec<u8>>> {
    let checkpoint = store
        .load_checkpoint(prior_claim_id, GUEST_STATE_STAGE_SENTINEL)
        .await?;
...
pub async fn save_guest_state(
    ...
    if blob.len() >= MAX_LOG_ENTRY_BYTES {
        return Err(PcsError::configuration(format!(
            "guest state blob size {} bytes exceeds MAX_LOG_ENTRY_BYTES {} — \
...
mod tests {
    ...
    fn sentinel_does_not_collide_with_the_accumulator_sentinel() {
        assert_ne!(GUEST_STATE_STAGE_SENTINEL, ACCUMULATOR_STAGE_SENTINEL);
    }

    #[tokio::test]
    async fn load_returns_none_on_first_run() {
        let store = MemStore::default();
        assert!(load_guest_state(&store, claim()).await.unwrap().is_none());
    }
    ...
    async fn save_then_load_returns_the_blob_verbatim() {
        ...
        save_guest_state(&store, id, &blob, 7).await.unwrap();
        assert_eq!(
            load_guest_state(&store, id).await.unwrap(),
            Some(blob.clone())
        );

        let stored = store
            .load_checkpoint(id, GUEST_STATE_STAGE_SENTINEL)
            .await
            .unwrap()
    ...
    async fn state_is_scoped_to_the_key_it_was_written_under() {
        ...
        save_guest_state(&store, a, &[1], 1).await.unwrap();
        assert_eq!(load_guest_state(&store, a).await.unwrap(), Some(vec![1]));
        assert!(
            load_guest_state(&store, b).await.unwrap().is_none(),
    ...
    async fn save_of_an_empty_blob_writes_nothing() {
        let store = MemStore::default();
        let id = claim();
        save_guest_state(&store, id, &[], 1).await.unwrap();
        assert!(
            store
                .load_checkpoint(id, GUEST_STATE_STAGE_SENTINEL)
                .await
    ...
    async fn stored_empty_payload_loads_as_none() {
        let store = MemStore::default();
        let id = claim();
        // Simulate a checkpoint written by an older build with an empty payload.
        store
            .save_checkpoint(id, GUEST_STATE_STAGE_SENTINEL, Vec::new(), 1)
            .await
            .unwrap();
        assert!(load_guest_state(&store, id).await.unwrap().is_none());
    }
    ...
    async fn save_rejects_an_oversized_blob() {
        ...
        let err = save_guest_state(&store, claim(), &blob, 1)
            .await
    ...
    async fn state_does_not_collide_with_accumulator_checkpoints() {
        ...
        store
            ...
            .await
            .unwrap();
        save_guest_state(&store, id, &[9, 9], 1).await.unwrap();
