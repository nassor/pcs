...
impl CsvSink {
    ...
    ///
    /// With `has_header`, the column names are written as the first row on the
    /// first `write_batch` call.
    ///
    /// # Errors
    ///
...
impl Sink for CsvSink {
    ...
    async fn finish(&mut self) -> Result<(), PcsError> {
        // Buffered through BufWriter; the flush happens on drop, and
        // arrow_csv::Writer has no explicit finish().
        Ok(())
    }
}
