...
//!
//! Reads a CSV file with [`arrow_csv::ReaderBuilder`] from an explicit schema
//! and an optional header row. A dedicated OS thread does the reading and
//! [`Source::next_batch`] awaits an `mpsc` channel, so the executor never
//! blocks and the file is not materialised up front.
...
impl CsvSource {
    ...
    ///
    /// `has_header` marks a first row of column names. It is skipped while
    /// reading; the supplied `schema` governs column types.
    ///
...
mod tests {
    ...
    /// Pull only the first batch; the reader thread handles the rest lazily.
    #[tokio::test]
