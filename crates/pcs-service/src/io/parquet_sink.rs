...
///
/// The [`ArrowWriter`] is held without a `Mutex`: [`Sink::write_batch`] and
/// [`Sink::finish`] both take `&mut self`, and `ArrowWriter<BufWriter<File>>`
/// derives `Sync`, so the `Sink: Send + Sync` bound holds without locking.
