...
//!
//! Reads a Parquet file with the synchronous
//! [`parquet::arrow::arrow_reader::ParquetRecordBatchReaderBuilder`] on a
//! dedicated OS thread, streaming each [`RecordBatch`] through a
//! [`tokio::sync::mpsc`] channel. [`Source::next_batch`] awaits the channel
//! instead of blocking the async executor, and the file is never materialised
//! in full. Parquet is self-describing, so the schema comes from file metadata
//! and no explicit schema is needed at construction.
...
/// RecordBatches are streamed one at a time from a background OS thread; the
...
impl ParquetSource {
    ...
    /// thread to stream batches. Batches are produced on demand via
    /// [`Source::next_batch`].
    ///
    /// # Errors
    ///
    ...
    /// Open the file, read metadata, and spawn the reader thread.
    fn open_sync(path: PathBuf) -> Result<Self, PcsError> {
        ...
        // Capacity 4 lets the reader stay slightly ahead without holding much
        // memory.
...
mod tests {
    ...
    /// Pull the first batch and confirm data arrives without reading the whole
    /// file.
    #[tokio::test]
    async fn test_parquet_source_streams_first_batch_without_full_materialisation() {
        let schema = make_schema();
        // 10 000 rows: enough to span several batches, still fast in CI.
        ...
        // The channel-based impl hands back the first batch without reading
        // further into the file.
        let mut src = ParquetSource::from_path(f.path()).unwrap();
        let first = src.next_batch().await.unwrap();
        assert!(first.is_some(), "first batch must not be None");
        // Drop the source; the reader thread sees the closed channel and exits.
    }
}
