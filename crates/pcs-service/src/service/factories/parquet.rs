...
//!
//! `ParquetSource` takes `path` and reads the schema from the file.
//! `ParquetSink` takes `path` plus `schema_fields`, an array of
//! `{name, type, nullable}` entries describing the output schema.
...
/// - `schema_fields` (list, required): Arrow schema for the output file. Each
///   entry is `{name: string, type: string, nullable: bool?}`.
