...
//!
//! [sources.config]
//! bind = "0.0.0.0:9500"       # required
//! buffer = 64                 # queued batches before backpressure (default 64)
//! max_frame_bytes = 8388608   # per-frame cap (default 8 MiB)
//!
...
//! ```
//!
//! Producers write `u32` big-endian length + one Arrow IPC stream payload
//! (schema header plus exactly one `RecordBatch`) per frame.
