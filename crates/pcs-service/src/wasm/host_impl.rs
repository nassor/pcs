...
///
/// Holds the config map injected at load time so the guest can call
/// `get-config` during `init` and `run-batch`. Config values are strings;
/// guests parse numerics themselves. The WASI context is here because
/// transitive deps (arrow-ipc, serde_arrow, std) require WASI imports.
pub struct HostState {
    /// Prefixes log output.
    pub name: String,
    /// Key/value config from the TOML `[pipeline.wasm.config]` table. Shared
    /// with the owning runtime, so a guest call costs one `Arc` bump and no map
    /// copy.
...
impl HostIo for HostState {
    ...
    fn metric(&mut self, name: String, value: f64) {
        ...
        // trace-level logging keeps them visible instead of dropping them.
        #[cfg(feature = "tracing")]
        {
