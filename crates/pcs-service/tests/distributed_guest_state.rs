// Integration test — DistributedRunner threads a runtime's state blob across batches
//
...
// mock can assert on exactly what `prior` it received, which a guest cannot
// report back. `tests/wasm_roundtrip.rs` covers the guest half against a real
// `.wasm`.
//
// The blob is written under the claim that produced it and read back under the
// claim that wrote the previous one — see `distributed::guest_state_store` for
// why the pointer has to be chained instead of derived from a stable partition
// key.
...
// ---------------------------------------------------------------------------
// MockCounterRuntime — a runtime whose entire state is a u64 batch counter
// ---------------------------------------------------------------------------
    ...
    async fn run_on_with_state(
        ...
        if n == self.fail_on_call {
            return Err(PcsError::SystemExecution(
                "guest trap (run-batch): injected failure".to_string(),
            ));
        }
...
// ---------------------------------------------------------------------------
// RecordingStore — RedbSharedStore plus release/ack counters and claim history
// ---------------------------------------------------------------------------
...
fn temp_db_path() -> std::path::PathBuf {
    std::env::temp_dir().join(format!("pcs_guest_state_{}.redb", Uuid::new_v4()))
}
...
async fn state_blob_carries_across_batches() {
    ...
    // Call 1 saw no prior; call 2 saw the blob call 1 returned.
    ...
    for (i, claim_id) in claims.iter().enumerate() {
        let stored = store
            .load_checkpoint(*claim_id, GUEST_STATE_STAGE_SENTINEL)
            .await
            .unwrap()
...
/// A runtime error from `run_on_with_state` releases the claim rather than
/// acking it, and writes no state — the same invariant `wasm_chaos.rs` pins for
/// the stateless path.
#[tokio::test]
