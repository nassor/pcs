//! Wire-format regression test for [`common::ToxiproxyClient`].
//!
...
//! so we can assert the client speaks the documented Toxiproxy HTTP API
//! (<https://github.com/Shopify/toxiproxy#http-api>). This is the only
//! coverage of `ToxiproxyClient` that runs in CI without a Toxiproxy
//! container — the chaos tests in `transport_chaos.rs` et al. exercise the
//! same client against a real daemon but soft-skip without Docker.
...
/// Every [`ToxiproxyClient`] method must issue the exact method/path/JSON
/// body the real Toxiproxy daemon expects. See
/// <https://github.com/Shopify/toxiproxy#http-api> and the toxic attribute
