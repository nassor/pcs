...
//! component. `scripts/ci/guest_ipc_roundtrip.sh` runs the two steps:
//!
...
//!   Host and guest both take the pinned `arrow-ipc = "=59.2.0"` through
//!   `workspace = true`, so differing bytes mean the pin has drifted on one side.
//! - **Config delivery.** The guest reads the `greeting` key through the
//!   `host-io` `get-config` import and appends it to the pipeline name, so
//!   `describe()` reports whether `[pipeline.wasm.config]` reached the guest.
...
/// schema-fingerprint check below is what enforces it.
...
/// guest state survives: the host builds a fresh wasmtime Store, and so fresh
/// guest linear memory, for every call.
#[tokio::test(flavor = "current_thread")]
...
/// Without the blob the guest starts from scratch on every batch.
#[tokio::test(flavor = "current_thread")]
