...
description = "Building the binary, writing the config, bootstrapping a three-node cluster, monitoring it, and recovering from the common failure modes."
template = "page.html"
+++
...
that open files (e.g. `CsvSink`, `ParquetSink`) create the output file
immediately, so the parent directory must exist before running `validate`.
...
Subsequent log lines depend on `log_format`. Confirm liveness:
...
| TOML `kind` | Behaviour |
|-------------|-----------|
| `continuous` (default) | Loop until cancelled; re-enters immediately when the source has work |
...
membership is managed manually in v1, and standalone with a good backup
...
Returns `200` when the service is ready to handle work, `503` otherwise. In v1
the ready flag flips as soon as the runner task is spawned, before the first
iteration completes.
...
**Cluster** (when `cluster_probe` is wired; see the v1 note below):
...
In v1 the serve command does not wire `cluster_probe`, so the `cluster` field
is `null` even in cluster mode. Full Raft metrics are planned for v1.1.
...
force-snapshot command in v1. If the file grows unexpectedly, check that
...
**Development** (default, colored and human-readable):
...
than an error, because dynamic membership is not implemented in v1.
