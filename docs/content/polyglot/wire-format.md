...
which is exactly what the Go, Python and JavaScript stages of
[the polyglot example](@/polyglot/_index.md) do.

Every number here was verified by parsing the stream that
...
8-byte alignment, no compression — containing exactly one Schema message, one
...
- A vtable is `u16 vtable_len`, `u16 table_len`, then one `u16` per field id. A
  field offset of `0` — or a field id at or beyond `vtable_len` — means the field
  is **absent**.
...
set). So the slot count is fixed by `type_type`, never inferred from lengths. A
nine-field schema of `Int, Utf8, Utf8, Float, Bool, Float, Float, Bool, Utf8`
consumes exactly 21 buffer slots; after the walk, the accumulated index must
...
never changes any length. The pattern the three non-Rust stages use:
...
4. read the columns it needs, then overwrite fixed-width value bytes **in place**
   in the body,
5. return the input byte array, mutated. Every other byte — the `__alive`
   segment, both flatbuffers, the framing — passes through untouched.
...
`format!("{:08x}", fnv1a32)` — lowercase, zero-padded to 8 characters. FNV-1a
...
`run-batch` returns `result<run-result, run-error>`, and the variant decides what
the host does with the batch:
...
guest panic or trap is surfaced as `permanent` too, but the operator loses the
batch — returning a structured error is strictly better.
