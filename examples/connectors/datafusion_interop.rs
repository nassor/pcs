...
//! [`SessionContext`] and streams the results into a PCS [`Dataset`], which a
//! [`Pipeline`] then aggregates.
//!
//! ```bash
//! cargo run -p pcs-connector-datafusion --example datafusion_interop
//! ```
...
use pcs_core::{PcsError, Pipeline, System, SystemMeta};
