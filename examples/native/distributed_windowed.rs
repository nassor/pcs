...
//!
//! State lives in TiKV, so several instances pointed at the same `key_prefix`
//! share one work pool; raise `num_instances` above 1 to shard keys across
//! them.
//!
//! Needs the `tikv-store` and `windows` features and a reachable TiKV: PD at
//! `PCS_PD_ENDPOINTS` (default `127.0.0.1:2379`). The example exits with a
//! clear message when the store cannot be reached.
//!
//! ```bash
//! cargo run --example distributed_windowed --features "tikv-store,windows"
//! ```
...
use pcs_service::distributed::{
    DistributedRunner, KeyPartition, RunnerConfig, TikvSharedStore, TikvStoreConfig,
};
...
async fn main() -> Result<(), PcsError> {
    println!("=== Distributed Windowed Aggregation (TiKV-backed) ===\n");
