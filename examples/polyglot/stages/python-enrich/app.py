...
and is never configurable; an unrecognised code converts 1:1, which is the
conservative choice --- it keeps the row visible to the risk stage instead of
...
re-initialises every batch --- the cost is real, and the reason a hot inner
loop belongs in the Rust or Go stage rather than here.

Build (from this directory):

    componentize-py -d ../../../../crates/pcs-guest/wit -w pcs-pipeline bindings .
    componentize-py -d ../../../../crates/pcs-guest/wit -w pcs-pipeline componentize app -o enrich-py.wasm
...
componentize-py resolves imports at build time only, so every import in this
file and in `arrow_ipc` is module level. A function-local import is a runtime
`ImportError` inside the component, not a build failure.
"""
...
_ORDER_SCHEMA_IPC = arrow_ipc.decode_schema_ipc(ORDER_SCHEMA_IPC_BASE64)
