//! Windowing demo as a WebAssembly processor.
//!
//! The wasm half of `examples/windowing/`: receives the merged rows of both
//! NATS sources (one batch per source per stream item), keeps open windows in
//! its checkpoint state, and emits one `WindowTotal` row per closed
//! `(window_id, symbol)` group — Beam's default trigger, where a window
//! closes when the event-time watermark passes its end. The geometry comes
//! from the `window.*` config keys the host injects from the KDL `window`
//! block; the plugin counterpart in `examples/windowing/plugin` runs the same
//! logic as a native plugin.
//!
//! # Build
//!
//! ```bash
...
//! `target/wasm32-wasip2/release/windowing_wasm.wasm`.
...
use pcs_processor::ProcessorState;
use pcs_processor::arrow_array::{Float64Array, Int64Array, StringArray};
use pcs_processor::arrow_schema::{DataType, Field, FieldRef, Schema};
use pcs_processor::prelude::*;
use pcs_processor::windows::WindowSpec;
...
/// One aggregate row the processor emits when a window closes.
#[derive(serde::Serialize, serde::Deserialize)]
...
pub fn build() -> Pipeline {
    let mut pipeline = Pipeline::new("windowing");
    pipeline
        .data
...
#[cfg(target_arch = "wasm32")]
pcs_processor::export_pipeline!(build, state = WindowState);

#[cfg(test)]
mod tests {
    use super::*;
    use pcs_processor::__rt::{ProcessorStateSpec, Stateful};
