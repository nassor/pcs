// Reads and mutates the PCS host<->guest wire format using nothing but the .NET
// base class library.
//
...
//
// It never writes a flatbuffer. Overwriting a fixed-width value slot is a read of
// the flatbuffer metadata plus a byte write into the body, which is all the base
// class library is needed for. SetInt64, SetFloat64 and SetBool therefore accept
// fixed-width fields only: changing a Utf8 value would shift every following
// offset and force a rewrite of the RecordBatch metadata.
//
// The `__alive` segment is addressable but never mutated: the host marks every
// row of a batch alive, and a guest that can neither add nor remove rows cannot
...
public static class ArrowIpc
{
    ...
    /// <remarks>A guest that embeds its component schema as a generated constant
...
/// <summary>A parsed PCS wire-format stream.</summary>
/// <remarks><see cref="Buffer"/> is the guest-owned mutable buffer: every Set call
/// writes into it, and it is what the guest hands back to the host as
/// `run-result.output`.</remarks>
public sealed class PcsStream
{
    ...
    /// only double a batch's footprint inside the guest.</remarks>
    ...
    {
        Column[] columns = SchemaFieldsOf(schema.Header);
...
public sealed class ArrowBatch
{
    ...
    internal ArrowBatch(int rows, string component, byte[] buf, Column[] columns)
    {
        Rows = rows;
    ...
    {
        ref Column c = ref Reader(name, ArrowIpc.TypeInt, 8);
    ...
    {
        ref Column c = ref Reader(name, ArrowIpc.TypeFloat, 8);
    ...
    {
        ref Column c = ref Field(name, ArrowIpc.TypeBool);
        RequireBits(ref c);
    ...
    {
        ref Column c = ref Field(name, ArrowIpc.TypeUtf8);
    ...
    {
        ref Column c = ref Writer(name, ArrowIpc.TypeInt, row, 8);
    ...
    {
        ref Column c = ref Writer(name, ArrowIpc.TypeFloat, row, 8);
    ...
    {
        ref Column c = ref Writer(name, ArrowIpc.TypeBool, row, 0);
    ...
    private ref Column Field(string name, byte want)
    {
        ref Column c = ref _columns[FieldIndex(name)];
        if (c.Type != want)
        {
            throw new ArrowIpcException(
    ...
    /// every row.</summary>
    private ref Column Reader(string name, byte want, int width)
    {
        ref Column c = ref Field(name, want);
    ...
    private ref Column Writer(string name, byte want, int row, int width)
    {
    ...
    private void RequireBits(ref Column c)
    {
