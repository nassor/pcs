...
public class ArrowIpcTests
{
    ...
    private const string EmitHint =
        "run `cargo run -p pcs-service --features wasm --example polyglot_orders -- emit` first";
    ...
    [
        "id", "region", "currency", "amount", "valid", "usd_amount",
    ...
    private static string FindRepoRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(dir.FullName, "crates", "pcs-guest", "wit", "pipeline.wit")))
            {
                return dir.FullName;
            }
        }
    ...
    /// <summary>Checks all eleven columns against the JSON ground truth.</summary>
    ...
    /// <remarks>The rule lives in PipelineExports.cs, which only compiles for
    ...
    /// <summary>`settlement` is the chain's one variable-length output and belongs to
    /// the Rust stage. Both fixed-width setters have to refuse it by name, and refuse
    /// it before touching anything.</summary>
    [Fact]
    ...
    /// <summary>The one encoding helper a guest needs for its schema constant.</summary>
    [Fact]
