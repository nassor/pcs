...
format a guest needs. The guest never *writes* a flatbuffer: it locates its
component's segment, reads the columns it consumes, overwrites fixed-width value
bytes in place, and hands the input buffer back. Framing, both flatbuffers, and
every column it did not touch are returned byte-identical.

`set_int64`, `set_float64` and `set_bool` are therefore the whole write surface.
A `Utf8` column cannot be written here at all: changing a string resizes the
values buffer and invalidates the offsets buffer *and* the RecordBatch
flatbuffer that describes both, which needs a real Arrow writer.

The `__alive` segment is parsed (so a malformed stream is still rejected) but
never consulted: a guest that can neither add nor remove rows cannot change
...
def decode_base64(encoded):
    ...
    Lives here so a guest that embeds its component schema as a generated
